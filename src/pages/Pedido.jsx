...
import telescopicos1 from "@/assets/telescopicos1.jpg";
...
const Pedido = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/pedidos/cliente/${currentUser.email}`
      );
      if (!response.ok) {
        if (response.status === 404) {
          // If 404 (Not Found), it means no orders for this user, return empty array
          return [];
        }
  ...
  const { data: viewedProducts } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/productosvistos/email/${currentUser.email}`
      );
      if (!response.ok) {
        if (response.status === 404) {
          return [];
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (dateRange === "últimos 3 meses") {
      ...
      orders = orders.filter(
        (order) => new Date(order[0].createdAt) >= threeMonthsAgo
      );
    } else if (dateRange !== "Todos") {
      // Handle year filters
      const year = parseInt(dateRange, 10);
      orders = orders.filter(
        (order) => new Date(order[0].createdAt).getFullYear() === year
      );
    }
    ...
    if (activeTab === "Pendiente de envío") {
      orders = orders.filter(
        (order) => order[0].estatus === "Pendiente de envío"
      );
    }
    ...
    if (searchTerm) {
      ...
      orders = orders.filter((order) => {
        const folioMatch = order[0].folio
          .toLowerCase()
          .includes(lowercasedSearchTerm);
        const itemMatch = order.some(
          (item) =>
  ...
  if (authLoading || !currentUser || isLoading || isLoadingProducts) {
    return (
      <div className={styles["loading-container"]}>Cargando pedidos...</div>
    );
  }
  ...
  const OrderCard = ({ orderItems, products }) => {
    ...
    const fechaEntrega =
      latestFechaEntrega.getTime() !== new Date(0).getTime()
    ...
    return (
      <div className={styles["order-card"]}>
        <div className={styles["card-header"]}>
          <div className={styles["header-left"]}>
            <div className={styles["header-item"]}>
              ...
              <span className={styles["header-value"]}>
                {formatDateToSpanish(orderItems[0].createdAt)}
              </span>
            </div>
            <div className={styles["header-item"]}>
              <span className={styles["header-label"]}>TOTAL</span>
              <span className={styles["header-value"]}>
                ${parseFloat(orderItems[0].total_pedido).toFixed(2)}
              </span>
            </div>
            <div className={styles["header-item"]}>
              <span className={styles["header-label"]}>ENVIAR A</span>
              <span className={styles["header-value"]}>
                {orderItems[0].enviar_a}
              </span>
            </div>
          </div>
        ...
        <div className={styles["card-footer"]}>
          <p>
            <span className={styles["footer-label"]}>Fecha de entrega:</span>{" "}
            {fechaEntrega}
          </p>
          <p className={styles["order-status"]}>
            Estatus: {orderItems[0].estatus}
          </p>
        </div>
      </div>
    );
      ...
      <div className={styles["top-section"]}>
        <div className={styles["contenedor-izq-top"]}>
          ...
          <div className={styles["tabs-menu"]}>
            ...
            <button
              className={`${styles["tab-button"]} ${
                activeTab === "Pendiente de envío" ? styles.active : ""
              }`}
