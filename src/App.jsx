...
const getReferrerInfo = (referrerUrl) => {
  if (!referrerUrl) return { isSearchEngine: false, searchEngineName: null, referrerDomain: null };
  ...
  try {
    ...
    for (const se of searchEngines) {
      if (se.domains.some(d => domain.includes(d))) {
        return { isSearchEngine: true, searchEngineName: se.name, referrerDomain: domain };
      }
    }
    return { isSearchEngine: false, searchEngineName: null, referrerDomain: domain };
  } catch (e) {
    console.error("Error parsing referrer URL:", e);
    return { isSearchEngine: false, searchEngineName: null, referrerDomain: referrerUrl };
  }
};
...
function App() {
  useEffect(() => {
    ...
    const sendAnalyticsData = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/analytics/visit`, {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          console.error('Failed to send analytics data:', response.status, response.statusText);
        }
      } catch (error) {
